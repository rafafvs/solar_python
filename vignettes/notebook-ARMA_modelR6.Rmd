...
# 100 step ahead
ARMA_variance(h = 10, A, b, sigma2 = 1)
```
AR variance
```{r}
# AR (1)
phi <- 0.4
ARMA_variance(1000, ARMA_companion_matrix(phi), ARMA_vector_b(1, 0))[1000]
AR_variance(phi, 1)
# AR (2)
phi <- c(phi, 0.1)
ARMA_variance(1000, ARMA_companion_matrix(phi), ARMA_vector_b(2, 0))[1000]
AR_variance(phi, 1)
# AR (3)
phi <- c(phi, 0.05)
ARMA_variance(1000, ARMA_companion_matrix(phi), ARMA_vector_b(3, 0))[1000]
AR_variance(phi, 1)
# AR (4) (no good)
phi <- c(phi, 0.03)
ARMA_variance(10000, ARMA_companion_matrix(phi), ARMA_vector_b(4, 0))[1000]
AR_variance(phi, 1)
```
ARMA covariances
```{r}
...
```
```{r}
fit <- ARMA_fit(x, arOrder = 1, maOrder = 2, method = c("CSS"))

c(fit$phi, fit$theta)
fit$std.errors$phi
fit$std.errors_rob$phi
```
...
arma$expectation(1000)[1000]
```

```{r}
# Create new coefficients
arma$variance(1000)[1000]
```
